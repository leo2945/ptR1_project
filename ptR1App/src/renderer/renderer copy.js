...
pc.onicecandidate = (e) => {
  if (e.candidate) {
    ...
    console.log('[ICE] ❄️ All candidates sent (null candidate)');
  }
};
...
async function startWebRTC() {
  ...
  try {
    ...
    console.log('[WebRTC] ❄️ ICE gathering complete');

    console.log('[SDP] 📤 Sending offer to MediaMTX:\n', pc.localDescription.sdp);

    // *** แก้ไขตรงนี้: เปลี่ยน Content-Type และ body ***
    const res = await fetch('http://localhost:8889/mystream/whep', {
      method: 'POST',
      headers: { 'Content-Type': 'application/sdp' }, // เปลี่ยนเป็น application/sdp
      body: pc.localDescription.sdp, // ส่งแค่ string SDP
    });

    console.log(`[WebRTC] 📬 Response status: ${res.status}`);
    if (!res.ok) {
        const errorText = await res.text(); // อ่าน error text เพื่อดูรายละเอียด
        throw new Error(`MediaMTX responded with status ${res.status}: ${errorText}`);
    }

    // *** แก้ไขตรงนี้: อ่าน response เป็น text และสร้าง RTCSessionDescription ใหม่ ***
    const answerSdp = await res.text(); // อ่าน response เป็น text
    console.log('[SDP] 📥 Answer from MediaMTX:\n', answerSdp);

    console.log('[WebRTC] 🧭 Setting remote description...');
    await pc.setRemoteDescription(new RTCSessionDescription({ type: 'answer', sdp: answerSdp })); // สร้าง RTCSessionDescription
