...
ipcMain.on('sync-maps', async () => {
  ...
  const localMapFiles = fs.readdirSync(localMapFolder)
  ...
  rosWorker.once('message', async (message) => {
    ...
    const imageArray = []; // ✅ เก็บภาพ base64 เพื่อนำไปแสดง
    const pendingMaps = [];
    ...
    for (const name of mapsToDownload) {
      ...
      await new Promise((resolve) => {
        rosWorker.once('message', (msg) => {
          if (msg.type === 'map-base64' && msg.data.name === name) {
            const buffer = Buffer.from(msg.data.base64, 'base64');
            const filePath = path.join(localMapFolder, `${msg.data.name}.png`);
...
ipcMain.handle('get-local-maps', async () => {
  const localMapFolder = path.join(app.getPath('userData'), 'maps');
  if (!fs.existsSync(localMapFolder)) return [];

  const files = fs.readdirSync(localMapFolder)
    .filter(file => file.endsWith('.png'))
    .map(file => {
      const fullPath = path.join(localMapFolder, file);
  ...
  // เรียงจากใหม่สุด (ถ้ามีการบันทึกด้วย timestamp)
...
ipcMain.handle('dialog:select-folder-map', async (event, defaultPath = null) => {
  const result = await dialog.showOpenDialog({
    properties: ['openFile'],
    filters: [{ name: 'Map Images', extensions: ['png'] }],
